...
@implementation ObjcGarbageCodeGenerator
- (VariableInfo *)localVar:(TypeInfo *)ti{
    ...
    if ([[ti type] isEqualToString:OBJC_TYPE_INT]) {
        ...
        [var setValue:value];
    } else if ([[ti type] isEqualToString:OBJC_TYPE_ID]) {
...
-(GarbageInfo *)generateGarbagePiece{
    ...
    if (arc4random()%3>0) {
        ...
        for (int i=0; i<count; i++) {
            NSMutableArray *classArr = [[DataBase shareInstance] classArrByClassType:CLASS_TYPE_OBJC];
            if ([classArr count]>0) {
                ...
                if ([methods count]>0) {
                    ...
                    if ([params count]>0) {
                        for (int j=0;j<[params count]; j++) {
                            ...
                            if ([[ti type] isEqualToString:OBJC_TYPE_BOOL]) {
                                [retn appendFormat:@"bool %@=%@;\n",[a name],[a value]];

                            } else if ([[ti type] isEqualToString:OBJC_TYPE_INT]) {
                                ...
                                [retn appendFormat:@"id %@=@\"%@\";\n",[a name],[a value]];

                            }
                            if ([paramStr length]>0) {
                                [paramStr appendFormat:@" %@:%@",[pi name],[a name]];
                            } else {
